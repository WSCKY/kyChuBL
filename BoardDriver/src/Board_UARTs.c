...
/*
 * @brief  Initializes Board UARTs.
 * @param  None
 * @retval Initial status.
 */
HAL_StatusTypeDef Board_UARTs_Init(void)
{
...
#if (UART7_ENABLE)
/*
 * @brief  send one byte use UART7 port.
 * @param  Data: data to send.
 * @retval None
 */
void Uart7PushOneByte(uint8_t Data)
{
	while((UART7->ISR & UART_FLAG_TXE) == RESET);
	UART7->TDR = (Data & (uint16_t)0x01FFU);
}
/*
...
#if (UART8_ENABLE)
/*
 * @brief  send one byte use UART8 port.
 * @param  Data: data to send.
 * @retval None
 */
void Uart8PushOneByte(uint8_t Data)
{
	while((UART8->ISR & UART_FLAG_TXE) != RESET);
	UART8->TDR = (Data & (uint16_t)0x01FFU);
}
/*
